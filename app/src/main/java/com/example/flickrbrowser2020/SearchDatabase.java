...
class SearchDatabase extends SQLiteOpenHelper {
     ...
     */
    static SearchDatabase getInstance(Context context) {
        if(instance == null) {
            Log.d(TAG, "getInstance: creating new instance");
            instance = new SearchDatabase(context);
        }
...
//        sSQL = "CREATE TABLE Tasks (_id INTEGER PRIMARY KEY NOT NULL, Name TEXT NOT NULL, Description TEXT, SortOrder INTEGER, CategoryID INTEGER);";
        ...
        sSQL = "CREATE TABLE " + TableContract.TABLE_NAME + " ("
                + TableContract.Columns._ID + " INTEGER PRIMARY KEY NOT NULL, "
                + TableContract.Columns.SUGGEST_COLUMN_TEXT_1 + " TEXT NOT NULL);";
        Log.d(TAG, sSQL);
        db.execSQL(sSQL);
